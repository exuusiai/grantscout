...
- `conflict_precision`, `conflict_recall`, `conflict_f1`: computed only when the
  query record contains `conflict_expected`.
...
for human factuality labels. SciFact conversion derives `conflict_expected` from
official `CONTRADICT` evidence labels. Disabling citation audit produces `N/A`
support precision rather than a misleading unchanged score.
