...
from grantscout.evaluation.qasper import persist_qasper_evaluation, run_qasper_answer_evaluation
from grantscout.evaluation.suite import persist_evaluation_suite, run_evaluation_suite
from grantscout.logging import configure_logging
...
from grantscout.proposal.templates import list_templates
...
from grantscout.reports.renderer import render_markdown
from grantscout.tools.search import search_papers
...
def ask(
    ...
    if corpus.exists():
        with CorpusStore(corpus) as store:
            if store.paper_count() > 0:
                state = GrantScoutAgent(store, settings).run(question)
    ...
    prompt = (
        "You are the GrantScout research assistant. Return a concise JSON object with "
...
def ingest(
    ...
    scope: str = typer.Option("public", help="Document scope: public or private."),
    corpus: Path = typer.Option(Path("data/corpus.sqlite"), help="SQLite corpus path."),
) -> None:
    """Parse a paper and add its sections and evidence to the local corpus."""
    if scope not in {"public", "private"}:
        _emit({"status": "error", "error": "scope must be public or private"})
    ...
    with CorpusStore(corpus) as store:
        ...
        store.set_paper_meta(document.paper.id, scope=scope)
        _emit(
            {
                "status": "ingested",
...
def benchmark(
    ...
    """Compare fixed RAG with GrantScout Agentic RAG."""
    if not corpus.exists():
        _emit({"status": "error", "error": f"Corpus does not exist: {corpus}"})
        raise typer.Exit(code=1)
...
def list_templates_command() -> None:
    """List available proposal templates and their section budgets."""
    _emit(
        {
            "templates": [
                {
                    ...
                    "name": template.name,
                    "sections": [
                        {"key": spec.key, "title": spec.title, "max_words": spec.max_words}
...
@app.command()
def propose(
    topic: str = typer.Argument(..., help="Proposal topic (research subject)."),
    template: str = typer.Option("project-plan-v1", help="Proposal template id."),
    locale: str = typer.Option("zh", help="Output language: zh or en."),
    corpus: Path = typer.Option(Path("data/corpus.sqlite"), help="SQLite corpus path."),
) -> None:
    """Draft a full proposal from the topic using the local literature corpus."""
    settings = get_settings()
    configure_logging(settings.log_level)
    settings.prepare_directories()
    if not corpus.exists():
        _emit({"status": "error", "error": f"Corpus does not exist: {corpus}"})
        raise typer.Exit(code=1)
    request = ProposalRequest(topic=topic, template_id=template, locale=locale)  # type: ignore[arg-type]
    try:
        with CorpusStore(corpus) as store:
            document = ProposalPipeline(store, settings, template_id=template, output_language=locale).run(request)
    ...
    _emit(
        {
            ...
            "sections": {
                ...
                for key, versions in document.sections.items()
                for draft in [versions[-1]]
            },
            "review": review.model_dump(mode="json") if review else None,
            "artifacts": {
                "json": str(Path(settings.runs_dir) / f"{document.id}.proposal.json"),
                "markdown": str(Path(settings.runs_dir) / f"{document.id}.proposal.md"),
            },
        }
    )
...
def serve(
    ...
    """Start the minimal GrantScout web UI and API."""
    import uvicorn

    uvicorn.run("grantscout.api.app:app", host=host, port=port, reload=False)
